...
### 2025-10-08: Устранение визуального шума на таймлайне
**Проблема:** На временной шкале возникает "визуальный шум" из-за большого количества идущих подряд коротких событий (длительностью в несколько секунд), таких как быстрое переключение между вкладками или окнами. Это создает "дребезг" из множества мелких, трудночитаемых отрезков, который мешает анализу.
**Решение:** Реализовать механизм **гибридной динамической агрегации событий** в `dataProcessor.js`.

**Алгоритм:**
1.  **Условие объединения:** Объединять все идущие подряд события, если их длительность меньше `durationThreshold` (например, 30 секунд). Агрегация останавливается, как только встречается "длинное" событие.
2.  **Создание мета-события:** Вместо последовательности коротких событий создается одно мета-событие.
    *   **`timestamp`**: Берется от самого первого события в последовательности.
    *   **`duration`**: Равна **общей длительности всего временного отрезка** от начала первого до конца последнего объединенного события (включая возможные `AFK` промежутки).
    *   **`data`**: Собирается подробная статистика по содержимому блока:
        *   `is_aggregated`: `true`.
        *   `title`: Формируется строка вида `"Мелкая активность (чистое время): приложение1 (время1), приложение2 (время2)"`.
        *   `apps`: Объект со статистикой вида `{ "app1": duration1, "app2": duration2 }`.
        *   `eventCount`: Общее число объединенных событий.
3.  **Обработка AFK:** События `AFK` внутри агрегируемого блока учитываются при расчете общей `duration` (для сохранения корректной длины блока на таймлайне), но **игнорируются** при подсчете "чистого времени" и статистики для `title` и `apps`.
4.  **Визуальное представление:** Мета-события должны визуально отличаться от обычных. Они будут иметь **единый нейтральный фон**, поверх которого будут нарисованы **тонкие вертикальные линии-разделители**, показывающие пропорции времени каждого вложенного приложения. Цветовое кодирование использоваться не будет.
